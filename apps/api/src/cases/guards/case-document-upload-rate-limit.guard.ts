...
export class CaseDocumentUploadRateLimitGuard implements CanActivate {
  ...
  private getLimit() {
    return Number(this.config.get<string>("DOCUMENT_UPLOAD_RATE_LIMIT_MAX") ?? defaultUploadLimit);
  }

  private getWindowMs() {
    return Number(
      this.config.get<string>("DOCUMENT_UPLOAD_RATE_LIMIT_WINDOW_MS") ?? defaultUploadWindowMs
    );
  }
}
