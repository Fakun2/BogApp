...
export class RbacService implements OnModuleInit {
  ...
  async getPermissionsForRole(roleCode: string) {
    if (roleCode === "owner") {
      return RBAC_PERMISSIONS.map((permission) => permission.code).filter(
        isTenantAssignablePermission
      );
    }
...
function normalizePermissionsForHierarchy(permissionCodes: string[], hierarchyLevel: number) {
  if (hierarchyLevel === 3) {
    return withoutOwnerOnlyPermissions(
      withAdminAccess(
        RBAC_PERMISSIONS.map((permission) => permission.code).filter(
          isTenantAssignablePermission
        )
      )
    );
  ...
  const basePermissions = withoutOwnerOnlyPermissions(
    withAdminAccess(permissionCodes).filter(isTenantAssignablePermission)
  );
...
function isAllowedOperationalPermission(permissionCode: string) {
  return (
    ...
    permissionCode === "currencies:read" ||
