...
export function CaseHearingsTable({
  ...
  return (
    <Card
      data-admin-surface
      className={`${adminSurfaceClassName} flex min-h-[260px] flex-col overflow-hidden border-0 shadow-[var(--admin-card-shadow)]`}
    >
      <AdminTableHeader
        actions={
          canCreate ? (
      ...
      />
      <CardContent className="flex min-h-0 flex-1 flex-col px-2 md:px-6">
  ...
  return (
    <TableBody className="[&_tr:last-child]:border-0">
      {hearings.map((hearing) => (
        <TableRow className="h-[66px] border-border/40 hover:bg-secondary/30" key={hearing.id}>
          ...
          <TableCell className="max-w-[360px] truncate px-4 py-4">
            {hearing.description}
          </TableCell>
          <TableCell className="px-4 py-4">
...
function MessageBody({
  ...
  return (
    <TableBody className="[&_tr:last-child]:border-0">
      <TableRow>
        <TableCell className={`h-24 px-4 text-center text-sm ${className}`} colSpan={columnCount}>
          {message}
        </TableCell>
      </TableRow>
    </TableBody>
