...
import { getCasesServerSession, listCasesServer } from "./_api/cases.server-api";
...
async function CasesPageContent({
  ...
  const casesResult = await loadCases(queryParams);

  return (
    <div className="flex h-[calc(100svh-104px)] min-h-0 flex-col gap-3 overflow-hidden md:h-[calc(100svh-152px)] md:gap-4">
      <CaseMetrics casesData={casesResult.data} />
      <CasesTableCard
...
async function loadCases(params: CasesQueryParams) {
  try {
    return {
      data: await listCasesServer(params),
      error: null
    };
  } catch (error) {
    return {
      data: undefined,
      error: error instanceof Error ? error : new Error("No se pudieron cargar los expedientes.")
    };
  }
}
...
function MetricSkeleton() {
  return (
    <div
      data-admin-surface
      className="rounded-md border-0 bg-card p-4 shadow-[var(--admin-card-shadow)]"
    >
